...
class obstacle(): 
    #default  
    def __init__(self,x): 
        self.top = cylinder(pos=(x,500,0),axis=(0,-235,0),radius=45,color=color.green) 
        self.bot = cylinder(pos=(x,-500,0),axis=(0,235,0),radius=45,color=color.green) 
    ...
    def moveObstacle(self,dt,Vx,screenRange): 
        ...
        if self.top.x+self.top.radius<-screenRange: 
            self.modifyGap(screenRange) 
            self.top.x = screenRange + self.top.radius 
            self.bot.x = screenRange + self.bot.radius 
